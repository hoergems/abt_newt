...
std::unique_ptr<solver::HistoricalData> PositionData::createChild(
    ...
    std::shared_ptr<shared::ManipulatorState> collided_state;
    std::tie(next_state, isLegal, collided, collided_state) = model_->makeNextState(manipulator_state_, rsAction);
    if (!isLegal) {
