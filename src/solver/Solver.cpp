...
long Solver::improvePolicy(BeliefNode *startNode, long numberOfHistories, long maximumDepth,
    ...
    cout << "Do multiple searches" << endl;    
    long actualNumHistories = multipleSearches(startNode,  
                                               sampler, 
                                               maximumDepth, 
                                               numberOfHistories,
                                               startTime + timeout);
    cout << "Done multiple searches" << endl;
...
BeliefNode *Solver::replenishChild(BeliefNode *currNode, Action const &action,
    ...
    if (options_->hasVerboseOutput) {
        cout << "Replenishing particles... " << deficit << endl;
        cout.flush();
    }
    ...
    if (nextParticles.empty()) {
        ...
        cout << "Particles size: " << nextParticles.size();
    }
    if (nextParticles.empty()) {
    ...
    for (HistoryEntry *entry : nextNode->particles_) {
        nextNode->removeParticle(entry);
    }
...
void Solver::singleSearch(BeliefNode *startNode, StateInfo *startStateInfo, long maximumDepth) {    
    ...
    firstEntry->registerNode(startNode, addParticle);    
