...
FCL_INCLUDE_DIR      := /usr/local/include/
OPENRAVE_INCLUDE_DIR := /usr/local/include/openrave-0.9
OMPL_INCLUDE_DIR     := /usr/local/include/
BOOST_INCLUDE_DIR    := /usr/include/
PYTHON_INCLUDE_DIR   := /usr/include/python2.7
TINYXML_INCLUDE_DIR  := /usr/local/include/
RBDL_INCLUDE_DIR     := /usr/local/include
RBDL_INTERFACE_INCLUDE_DIR := /usr/local/include
URDF_INCLUDE_DIR     := /opt/ros/indigo/include
VIEWER_INTERFACE_INCLUDE_DIR := /usr/local/include
 

OMPL_LIB_DIR         := /usr/local/lib
FCL_LIB_DIR          := /usr/local/lib
OPENRAVE_LIB_DIR     := /usr/local/lib
BOOST_LIB_DIR        := /usr/lib/i386-linux-gnu
TINYXML_LIB_DIR      := /usr/local/lib
RBDL_LIB_DIR         := /usr/local/lib/x86_64-linux-gnu
RBDL_INTERFACE_LIB_DIR := /usr/local/lib
URDF_LIB_DIR         := /opt/ros/indigo/lib
VIEWER_INTERFACE_LIB_DIR := /usr/local/lib
...
# ----------------------------------------------------------------------
# Preprocessor flags.
override INCDIRS     += -I$(TINYXML_INCLUDE_DIR) -I$(FCL_INCLUDE_DIR) -I$(OMPL_INCLUDE_DIR) -I$(EIGEN3_INCLUDE_DIR) -I$(BOOST_INCLUDE_DIR) -I/usr/include/jsoncpp/ -I$(ROOT)/include -I$(ROOT)/src -I$(ROOT)/src/options -I$(PYTHON_INCLUDE_DIR) -I$(RBDL_INCLUDE_DIR) -I$(RBDL_INTERFACE_INCLUDE_DIR)
...
# Linker flags
# ----------------------------------------------------------------------
override LIBDIRS += -L/usr/lib/ -L/usr/local/lib/ -L/opt/ros/indigo/lib
override LIBDIRS += -L$(OMPL_LIB_DIR) -L$(FCL_LIB_DIR) -L$(BOOST_LIB_DIR) -L$(TINYXML_LIB_DIR) -L$(RBDL_LIB_DIR) -L$(RBDL_INTERFACE_LIB_DIR)


override LDFLAGS += $(LIBDIRS) -flto -O3 -fuse-linker-plugin
POSTFLAGS := -L$(PYLIBPATH) $(shell python-config --libs) -lfcl -lompl -Wl,-rpath /usr/local/lib/ -lopenrave0.9 -lopenrave0.9-core -lboost_system -lboost_program_options -lboost_python -lboost_thread -lboost_random -ltinyxml -lrbdl -lrbdl_interface

ifeq ($(USE_URDF),true)
  override CPPFLAGS  += -DUSE_URDF
  override INCDIRS   += -I$(OPENRAVE_INCLUDE_DIR) -I$(URDF_INCLUDE_DIR) -I$(VIEWER_INTERFACE_INCLUDE_DIR)
...
LINK_RECIPE    = $(CXX) $(LDFLAGS) $(1) $(LDLIBS) -o $@ $(POSTFLAGS)
